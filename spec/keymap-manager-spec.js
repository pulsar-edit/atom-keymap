...
describe("KeymapManager", function() {
  ...
  describe("::keystrokeForKeyboardEvent(event)", function() {
    ...
    describe("when a modifier key is combined with a non-modifier key", () => it("returns a string that identifies the modified keystroke", function() {
      ...
      return assert.equal(keymapManager.keystrokeForKeyboardEvent(buildKeydownEvent({key: '{', shiftKey: true})), '{');
    }));
    ...
    describe("when the KeyboardEvent.key is '' but the KeyboardEvent.code is 'NumpadDecimal' and getModifierState('NumLock') returns false", () => it("translates as delete to work around a Chrome bug on Linux", function() {
      ...
      return assert.equal(keymapManager.keystrokeForKeyboardEvent(buildKeydownEvent({key: '', code: 'NumpadDecimal', modifierState: {NumLock: false}})), 'delete');
    }));
    ...
    describe("when numlock is off", () => it("doesn't translate numpad digits using KeyboardEvent.code", function() {
      ...
      return assert.equal(keymapManager.keystrokeForKeyboardEvent(buildKeydownEvent({key: 'PageUp', code: 'Numpad9', modifierState: {NumLock: false}})), 'pageup');
    }));

    describe("when the Dvorak QWERTY-⌘ layout is in use on macOS", () => it("uses the US layout equivalent when the command key is held down", function() {
      mockProcessPlatform('darwin');
      ...
      return assert.equal(keymapManager.keystrokeForKeyboardEvent(buildKeydownEvent({key: 'L', code: 'KeyP', metaKey: true, shiftKey: true})), 'shift-cmd-P');
    }));

    describe("when a custom Dvorak QWERTY-⌘ layout is in use on macOS", () => it("uses the US layout equivalent when the command key is held down", function() {
      mockProcessPlatform('darwin');
      stub(KeyboardLayout, 'getCurrentKeymap', () => require('./helpers/keymaps/mac-dvorak-qwerty-cmd'));
    ...
    describe("when the current system keymap cannot be obtained on macOS", () => it("does not throw exceptions and just takes the current key value", function() {
      mockProcessPlatform('darwin');
      ...
      return assert.equal(keymapManager.keystrokeForKeyboardEvent(buildKeydownEvent({key: 'Dead', code: 'KeyU', modifierState: {AltGraph: true}})), 'alt-dead');
    }));
    ...
    describe("international layouts", function() {
      ...
      it("resolves events with a key value of Unknown and a code of IntlRo to '/' (this occurs on a Brazillian Portuguese keyboard layout on Mint Linux)", function() {
        mockProcessPlatform('linux');
        return assert.equal(keymapManager.keystrokeForKeyboardEvent(buildKeydownEvent({key: 'Unidentified', code: 'IntlRo', ctrlKey: true})), 'ctrl-/');
      });
      ...
      return it("translates dead keys to their printable equivalents on macOS, but not Windows", function() {
        ...
        return assert.equal(keymapManager.keystrokeForKeyboardEvent(buildKeydownEvent({key: 'Dead', code: 'BracketRight', ctrlKey: true, altKey: true, shiftKey: true})), 'ctrl-alt-shift-dead');
      });
    });

    return describe("when custom keystroke resolvers are installed", () => it("resolves to the keystroke string of the most recently-installed resolver returning a defined value", function() {
      mockProcessPlatform('darwin');
      ...
      const keydownEvent = buildKeydownEvent({key: '@', code: 'KeyG', ctrlKey: true, altKey: true});
      const disposable1 = keymapManager.addKeystrokeResolver(function({keystroke, event, layoutName, keymap}) {
      ...
      const disposable2 = keymapManager.addKeystrokeResolver(function({keystroke}) {
      ...
      const disposable3 = keymapManager.addKeystrokeResolver(function({keystroke}) {
...
`
        ...
        describe("when the file is moved", () => it("removes the bindings", function(done) {
          const newFilePath = path.join(temp.mkdirSync('keymap-manager-spec'), "other-guy.cson");
